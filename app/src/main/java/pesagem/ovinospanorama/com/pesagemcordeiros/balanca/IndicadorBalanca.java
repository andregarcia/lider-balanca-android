...
public class IndicadorBalanca {

    private static final String path = "lib\\Indicador.dll";
    private static String porta = "COM4";
    private static int baudRate = 9600;

    public static Indicador getIndicador() {

        //Carrega a Indicador.dll
        Indicador ind = (Indicador) Native.loadLibrary(path, Indicador.class);
        //Abre a porta serial passando paremtros
        boolean status = ind.openSerial(porta, baudRate); // ou ind.openTcp("192.168.0.201",3500) exemplo ;

        //Checa se a conexão foi aberta
        if (status) {
            return ind;
        }
        return null;
//            //Loop checando se a conexão está aberta, para sair do loop chama a função ind.close()
//            while (ind.isOpen()) {
//
//                System.out.println("Peso Bruto: " + ind.getPesoBruto() + " Peso Liquido: " + ind.getPeso() + " Tara: " + ind.getTara() + " Status: " + ind.getStatus());
//
//            /*
//             * Função waiting()
...
//        }// fim do statement if
    ...
    public static Indicador getIndicadorBalancaMock() {
        return new IndicadorMock();
    }

    //Interface do Indicador Virtual
